...
double evaluateNode(
  ...
  switch (node.kind) {
    ...
    case NodeKind::Add: return argument(0) + argument(1);
    case NodeKind::Subtract: return argument(0) - argument(1);
    case NodeKind::Multiply: return argument(0) * argument(1);
    ...
    case NodeKind::Power: return std::pow(argument(0), argument(1));
    case NodeKind::Negate: return -argument(0);
...
std::unordered_map<std::string, double> bindings(
  ...
  std::unordered_map<std::string, double> result{
      {"x", sample.x}, {"y", sample.y}, {"z", sample.z}, {"t", sample.t}};
  for (const Parameter& parameter : preset.parameters) result.emplace(parameter.name, parameter.value);
...
double ScalarExpression::evaluate(
...
std::vector<EquationPreset> builtInPresets() {
  return {
      {"wave-field", "Wave Field", "Traveling scalar wave for first visual and GPU-field validation.",
       {"amplitude * sin(wavenumber * x - angular_frequency * t)"}, {"x", "t"},
       {{"amplitude", 1.0, "field", 0.0, 10.0}, {"wavenumber", 2.0, "rad/m", 0.01, 50.0}, {"angular_frequency", 3.0, "rad/s", 0.01, 50.0}}},
      {"gravity-potential", "Gravity Potential", "Softened Newtonian potential; the softening avoids a singularity at the origin.",
       {"-gravitational_parameter / sqrt(x*x + y*y + z*z + softening*softening)"}, {"x", "y", "z"},
       {{"gravitational_parameter", 8.0, "m3/s2", 0.001, 1000.0}, {"softening", 0.25, "m", 0.001, 10.0}}},
      {"quantum-wavepacket", "Quantum Wavepacket", "A moving Gaussian-envelope carrier wave for probability-density visualizations.",
       {"exp(-((x - velocity*t)*(x - velocity*t)) / (2*width*width)) * cos(wavenumber*x - angular_frequency*t)"}, {"x", "t"},
       {{"velocity", 0.6, "m/s", -10.0, 10.0}, {"width", 1.2, "m", 0.01, 20.0}, {"wavenumber", 5.0, "rad/m", 0.01, 100.0}, {"angular_frequency", 3.0, "rad/s", 0.01, 100.0}}},
      {"electromagnetic-pulse", "Electromagnetic Pulse", "Damped oscillating field used to validate high-dynamic-range transfer functions.",
       {"amplitude * exp(-decay*t) * sin(wavenumber*x - angular_frequency*t)"}, {"x", "t"},
       {{"amplitude", 1.0, "field", 0.0, 10.0}, {"decay", 0.3, "1/s", 0.0, 10.0}, {"wavenumber", 4.0, "rad/m", 0.01, 100.0}, {"angular_frequency", 5.0, "rad/s", 0.01, 100.0}}},
      {"reaction-diffusion-seed", "Reaction Diffusion Seed", "Deterministic activator seed used as an input field for a later ping-pong simulation pass.",
       {"seed_strength * exp(-(x*x + y*y) / (2*radius*radius))"}, {"x", "y"},
       {{"seed_strength", 1.0, "concentration", 0.0, 1.0}, {"radius", 0.75, "m", 0.01, 20.0},
        {"diffusion_a", 1.0, "m2/s", 0.0, 2.0}, {"diffusion_b", 0.5, "m2/s", 0.0, 2.0},
        {"feed", 0.0367, "1/s", 0.0, 0.1}, {"kill", 0.0649, "1/s", 0.0, 0.1}}},
      {"buoyant-smoke", "Buoyant Smoke", "2D incompressible smoke with pressure projection, buoyancy, and vorticity confinement.",
       {"exp(-(x*x + y*y))"}, {"x", "y"},
       {{"buoyancy", 2.0, "m/s2", 0.0, 8.0}, {"vorticity", 0.35, "m2/s", 0.0, 3.0},
        {"density_dissipation", 0.998, "ratio", 0.9, 1.0}}},
      {"nbody-orbits", "N-Body Orbits", "Deterministic softened Newtonian N-body system integrated with velocity Verlet.",
       {"-central_mass / sqrt(x*x + y*y + softening*softening)"}, {"x", "y"},
       {{"central_mass", 100.0, "mass", 1.0, 500.0}, {"orbiter_mass", 1.0, "mass", 0.01, 20.0},
        {"softening", 0.05, "m", 0.005, 1.0}}},
      {"schwarzschild-lensing", "Schwarzschild Lensing", "Reference-guided Schwarzschild lensing preview using an RK4 deflection lookup.",
       {"1 / sqrt(x*x + y*y + 0.01)"}, {"x", "y"}, {}},
  };
}

std::optional<EquationPreset> findPreset(const std::string& id) {
  const auto presets = builtInPresets();
  const auto found = std::find_if(presets.begin(), presets.end(), [&](const EquationPreset& preset) {
    return preset.id == id;
  });
...
PresetRunSummary runPreset(const EquationPreset& preset, const PresetRunConfig& config) {
  if (config.frames == 0 || config.samplesPerFrame == 0 || config.timestepSeconds <= 0.0) {
    throw std::invalid_argument("preset run configuration must contain positive frames, samples, and timestep");
  }
  ...
  const double sampleDenominator = static_cast<double>(config.samplesPerFrame - 1u == 0 ? 1u : config.samplesPerFrame - 1u);
  for (uint32_t frame = 0; frame < config.frames; ++frame) {
    for (uint32_t index = 0; index < config.samplesPerFrame; ++index) {
      ...
      const Sample sample{normalized * 8.0 - 4.0, std::sin(normalized * 2.0 * std::numbers::pi), 0.5 * std::cos(normalized * 2.0 * std::numbers::pi), frame * config.timestepSeconds};
      for (const double value : evaluatePreset(preset, sample).values) {
        if (!std::isfinite(value)) throw std::runtime_error("preset produced a non-finite value: " + preset.id);
  ...
  const double count = static_cast<double>(config.frames) * config.samplesPerFrame * preset.expressions.size();
