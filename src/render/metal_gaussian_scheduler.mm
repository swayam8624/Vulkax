...
kernel void gaussian_schedule_csr(
    ...
    device uint* references [[buffer(3)]],
    device uint4* metadata [[buffer(4)]],
    ...
    uint total = 0u;
    for (uint splat = 0u; splat < projectedCount; ++splat) {
    ...
    // projected[] is already the stable back-to-front projection oracle order.
    // Emitting in increasing splat index therefore preserves both depth order
    // and equal-depth stability inside every tile.
    for (uint splat = 0u; splat < projectedCount; ++splat) {
        const float4 bounds = projected[splat].tileBounds;
        const uint firstColumn = uint(bounds.x + 0.5f);
        const uint lastColumn = uint(bounds.y + 0.5f);
    ...
    metadata[0] = uint4(total, maximum, 0u, 0u);
        ...
        id<MTLFunction> function = [library newFunctionWithName:@"gaussian_schedule_csr"];
        if (!function) throw std::runtime_error("Metal Gaussian scheduler function is missing");
        id<MTLComputePipelineState> pipeline = [device newComputePipelineStateWithFunction:function error:&error];
        if (!pipeline)
            throw std::runtime_error(std::string("Metal Gaussian scheduler pipeline failed: ") +
                                     [[error localizedDescription] UTF8String]);
        ...
        id<MTLBuffer> offsetsBuffer = [device newBufferWithLength:offsetsBytes
                                                          options:MTLResourceStorageModeShared];
        id<MTLBuffer> cursorsBuffer = [device newBufferWithLength:cursorsBytes
                                                          options:MTLResourceStorageModeShared];
        id<MTLBuffer> referencesBuffer = [device newBufferWithLength:referencesBytes
                                                             options:MTLResourceStorageModeShared];
        id<MTLBuffer> metadataBuffer = [device newBufferWithLength:4U * sizeof(std::uint32_t)
                                                           options:MTLResourceStorageModeShared];
        if (!projectedBuffer || !offsetsBuffer || !cursorsBuffer || !referencesBuffer || !metadataBuffer)
        ...
        [encoder setBuffer:cursorsBuffer offset:0 atIndex:2];
        [encoder setBuffer:referencesBuffer offset:0 atIndex:3];
        [encoder setBuffer:metadataBuffer offset:0 atIndex:4];
        [encoder setBytes:parameters.data() length:sizeof(parameters) atIndex:5];
        ...
        std::memcpy(schedule.tileOffsets.data(), [offsetsBuffer contents], offsetsBytes);
        schedule.projectedSplatIndices.resize(schedule.splatReferences);
        if (schedule.splatReferences > 0U) {
            std::memcpy(schedule.projectedSplatIndices.data(), [referencesBuffer contents],
