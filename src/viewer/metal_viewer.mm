...
@private
    ...
    ViewMode _mode;
    WorldState _state;
    BOOL _highlight, _showGrid, _autoOrbit, _visibilityDirty;
...
- (instancetype)initWithView:(MTKView*)view scene:(ViewerScene)scene {
    ...
    _highlight = YES; _showGrid = YES; _autoOrbit = NO; _visibilityDirty = YES;
...
- (void)updateStats {
    ...
    _stats.stringValue=[NSString stringWithFormat:@"Gaussians   %lu → %lu\nVisible     %lu | %lu\nCulled      %lu | %lu\nBudget      %lu\nParticles   %lu\nRewrite     %lu\nSurface     %@\nMax Δ       %.3e\nFPS         %.1f\n\nDrag .ply onto viewport\n1–5 modes · B/A state\nH highlight · G grid\nSpace orbit · R camera",
        ...
        (unsigned long)_scene.rewriteParticleCount,ns(_scene.surfaceKind),_scene.maxGaussianDisplacement,_fps];
}
...
- (void)handleKey:(NSString*)k {
    ...
    else if([k isEqualToString:@"3"])_mode=ViewMode::Surface; else if([k isEqualToString:@"4"])_mode=ViewMode::Particles; else if([k isEqualToString:@"5"])_mode=ViewMode::Compare;
...
- (void)updateVisibilityForView:(MTKView*)view {
    ...
    vulkax::viewer::ViewerCamera camera; camera.position=eye; camera.target=_target; camera.aspect=view.drawableSize.width/std::max(view.drawableSize.height,1.0);
...
- (void)drawSurface:(id<MTLRenderCommandEncoder>)e view:(MTKView*)view {
    if(!_surfaceBuffer||_scene.surfaceTriangles.empty())return; GPUUniforms u=[self uniforms:view particles:NO];[e setRenderPipelineState:_surfacePipeline];[e setDepthStencilState:_opaqueDepth];[e setVertexBuffer:_surfaceBuffer offset:0 atIndex:0];[e setVertexBytes:&u length:sizeof(u) atIndex:2];[e setFragmentBytes:&u length:sizeof(u) atIndex:2];[e drawPrimitives:MTLPrimitiveTypeTriangle vertexStart:0 vertexCount:_scene.surfaceTriangles.size()];
}
- (void)drawInMTKView:(MTKView*)view {
    ...
    [e setScissorRect:scissor(0,0,width,height)];
    ...
    if(_mode==ViewMode::Compare){
        [self drawSurface:e view:view];
        const NSUInteger half=std::max<NSUInteger>(1,width/2);
        [e setScissorRect:scissor(0,0,half,height)]; [self drawSplats:e view:view buffer:_beforeBuffer order:_beforeOrderBuffer count:_beforeVisible particles:NO];
        [e setScissorRect:scissor(half,0,width-half,height)]; [self drawSplats:e view:view buffer:_afterBuffer order:_afterOrderBuffer count:_afterVisible particles:NO];
        [e setScissorRect:scissor(0,0,width,height)];
    } else {
        if(_mode==ViewMode::Hybrid||_mode==ViewMode::Surface)[self drawSurface:e view:view];
        if(_mode==ViewMode::Hybrid||_mode==ViewMode::Splats){const BOOL before=_state==WorldState::Before;[self drawSplats:e view:view buffer:before?_beforeBuffer:_afterBuffer order:before?_beforeOrderBuffer:_afterOrderBuffer count:before?_beforeVisible:_afterVisible particles:NO];}
