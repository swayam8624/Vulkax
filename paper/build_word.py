#!/usr/bin/env python3
"""Build the editable Reality Probe Word manuscript from paper/main.tex.

The ACM/TOG PDF is the publication-layout source of truth. This exporter only
materializes references that Pandoc cannot resolve and cleans two math-conversion
edge cases; it does not change any scientific values.
"""
...
TEX = ROOT / "main.tex"
OUT = ROOT / "Reality_Probe_Manuscript.docx"

src = TEX.read_text(encoding="utf-8")
src = src.replace(
    "Eq.~\\ref{eq:second} is explanatory rather than the literal\nselected stencil.",
    "The second-difference expression above is explanatory rather than the literal\nselected stencil.",
)
src = src.replace(
    "Table~\\ref{tab:stages} summarizes the\ncentral outcomes.",
...
for para in doc.paragraphs:
    if "sum_i w_i F_M(u_i)" in para.text:
        ...
        para.clear()
        para.add_run(
            "The evidence denominator separates measurement, repeat, and numerical uncertainty. "
            "Independent measurement and repeat variance propagate through a signed stencil with "
            "the squared ℓ₂ weight gain Σᵢ wᵢ². Numerical error is treated more conservatively "
            "because discretization error can be correlated across intervention points. D3 "
            "additionally estimates numerical uncertainty directly in witness space by evaluating "
            "the same signed witness at nominal and refined solver resolution."
        )

props = doc.core_properties
props.title = "Reality Probe: Information-Limited Physical Verification of Captured Worlds"
props.author = "Swayam Singal"
props.subject = "Reality Probe / Vulkax research manuscript"
