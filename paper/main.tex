...
Abstention also has a long history. Classical reject-option analysis characterizes the trade-off between recognition error and rejection \citep{chow1970reject}, while modern selective prediction learns or calibrates rejection mechanisms to control predictive risk and coverage \citep{geifman2019selectivenet}. Reality Probe does not claim abstention as a new decision-theory primitive. UNRESOLVED is the rewrite transaction's explicit record that a reserved physical experiment failed to separate the baseline and candidate by the declared evidence margin.
