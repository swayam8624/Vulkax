#!/usr/bin/env bash
set -Eeuo pipefail
trap 'rc=$?; echo "[freefall-v67-smoke] ERROR rc=$rc line=$LINENO command=$BASH_COMMAND" >&2; exit $rc' ERR
...
case "$TAKE" in
  06|07|08|09|10) ;;
  *) echo "[freefall-v67-smoke] development take must be one of 06..10" >&2; exit 2;;
esac

VENV="$BUILD/.venv-iris-freefall"
PUBLIC="$BUILD/publication-validation/public-data"
OUT="$BUILD/publication-validation/iris-freefall-v67-smoke-$TAKE"
TMP_CFG="$(mktemp -t vulkax-v67-smoke.XXXXXX.json)"
...
assert cfg["tracker"]["revision"]=="ball_identity_v6_7", cfg["tracker"]["revision"]
...
if [[ ! -d "$PUBLIC/adapted/iris" ]]; then
  echo "[freefall-v67-smoke] adapted development data missing; this smoke command never downloads data." >&2
  echo "[freefall-v67-smoke] run the normal development materialization once, then retry." >&2
  exit 2
fi

rm -rf "$OUT"
echo "[freefall-v67-smoke] analyzing development-only drop_50/$TAKE; validation is forbidden"
...
if [[ ! -s "$OUT/summary.json" ]]; then
  echo "[freefall-v67-smoke] analyzer failed before summary generation (rc=$RC)" >&2
...
print("\n=== V6.7 ONE-TAKE DEVELOPMENT SMOKE ===")
for k in (
...
echo "[freefall-v67-smoke] completed; scientific gate may pass or fail without being treated as a software crash."
exit 0
