...
def main():
    ...
    p.add_argument("--geometry-mode",choices=("measured_aspect","square_cross","asset_bbox"),default="measured_aspect")
    ...
    asset_geometry=None
    if a.geometry_mode=="asset_bbox":
        if not a.asset_geometry_json:
            raise SystemExit("--asset-geometry-json is required for asset_bbox geometry")
        asset_geometry=json.loads(pathlib.Path(a.asset_geometry_json).read_text())
        if asset_geometry.get("provenance")!="published-asset+measured-metadata":
            raise SystemExit("asset geometry must come from the published GAUGE asset audit")
        if len(asset_geometry["task_translation_m"])!=3 or len(asset_geometry["asset_bbox"]["extent_m"])!=3:
            raise SystemExit("invalid asset geometry dimensions")
    commands=[]
    for material in ("soft","hard"):
        ...
        if asset_geometry is not None:
            ...
            cmd.extend(repr(float(x)) for x in asset_geometry["asset_bbox"]["extent_m"])
    ...
    manifest={
      ...
      "structural_controls":{
        ...
        "asset_geometry":None if asset_geometry is None else {
          ...
          "extent_m":asset_geometry["asset_bbox"]["extent_m"]
        }
      },
      "runs":commands,
