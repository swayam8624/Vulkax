...
def discrepancy(measured,predicted,key):
    ...
    measured_peak=max((abs(x) for x in a),default=0.0)
    predicted_peak=max((abs(x) for x in b),default=0.0)
    scale=max(measured_peak,1e-12)
    return {
        "rmse":e,
        "nrmse_to_measured_peak_abs":e/scale,
        "measured_peak_abs":measured_peak,
        "predicted_peak_abs":predicted_peak,
        "predicted_to_measured_peak_ratio":predicted_peak/scale,
        "correlation":correlation(a,b),
        "measured_final":a[-1] if a else None,
        "predicted_final":b[-1] if b else None,
        "final_sign_agrees":(
            True if not a or not b or abs(a[-1])<=1e-12
            else (a[-1]*b[-1]>0.0)
        ),
    }
...
def main():
    ...
    for material in ("soft","hard"):
        ...
        result["materials"][material]={
            ...
            "longitudinal_mean_final_sign_agrees":disc["longitudinal_strain_mean"]["final_sign_agrees"],
        }
