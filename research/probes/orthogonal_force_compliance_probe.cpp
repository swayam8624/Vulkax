...
struct Candidate {
    ...
    std::vector<Response> forceResponses;
    std::vector<Response> refinedForceResponses;
...
std::vector<Vec3> forceDirections(){
    return {
        { kForcePerTopParticle,0.,0.},
        {-kForcePerTopParticle,0.,0.},
        {0., kForcePerTopParticle,0.},
        {0.,0., kForcePerTopParticle}
    };
}
Response simulateForceCompliance(
    double E,double nu,MpmTransferScheme scheme,double dt,Vec3 force){
    auto particles=body();
    const auto initial=particles;
    const auto g=grid();
    MpmMaterial material{1000.,E,nu};
    constexpr double bottomY=-0.18;
    constexpr double topY=0.18;
    const auto steps=static_cast<std::size_t>(std::llround(kForceHorizon/dt));
    if(steps==0)throw std::runtime_error("force-compliance horizon too short");

    std::vector<PrescribedParticleTarget> bottom;
    for(const auto&p:initial){
        if(std::abs(p.restPosition.y-bottomY)<1e-12)
            bottom.push_back({p.id,p.restPosition,{0.,0.,0.},true});
    }
    if(bottom.empty())throw std::runtime_error("force-compliance missing fixed layer");

    for(std::size_t step=0;step<steps;++step){
        for(std::size_t i=0;i<particles.size();++i){
            particles[i].externalForce={0.,0.,0.};
            if(std::abs(initial[i].restPosition.y-topY)<1e-12)
                particles[i].externalForce=force;
        }
        (void)vulkax::research::stepMpmWithPrescribedParticles(
            particles,g,material,dt,bottom,{0.,0.,0.},scheme);
    }

    Vec3 top{},interior{};
    std::size_t topCount=0,interiorCount=0;
    for(std::size_t i=0;i<particles.size();++i){
        const auto d=particles[i].position-initial[i].restPosition;
        if(std::abs(initial[i].restPosition.y-topY)<1e-12){
            top+=d;++topCount;
        }else if(initial[i].restPosition.y>bottomY+1e-12 &&
                 initial[i].restPosition.y<topY-1e-12){
            interior+=d;++interiorCount;
        }
    }
    if(topCount==0||interiorCount==0)
        throw std::runtime_error("force-compliance empty observation region");
    top=top/static_cast<double>(topCount);
    interior=interior/static_cast<double>(interiorCount);
    return {top.x,top.y,top.z,interior.x,interior.y,interior.z};
}
std::vector<Response> simulateForceBundle(
    double E,double nu,MpmTransferScheme scheme,double dt){
    std::vector<Response> out;
    const auto directions=forceDirections();
    out.reserve(directions.size());
    for(const auto f:directions)
        out.push_back(simulateForceCompliance(E,nu,scheme,dt,f));
    return out;
}
double forceProgress(
    const Candidate&a,const Candidate&b,const std::vector<Response>&truthForce){
    const double ea=datasetRms(a.forceResponses,truthForce);
    const double eb=datasetRms(b.forceResponses,truthForce);
    const double na=datasetRms(a.forceResponses,a.refinedForceResponses);
    const double nb=datasetRms(b.forceResponses,b.refinedForceResponses);
    const double sigma=std::sqrt(std::max(
        2.*kNoise*kNoise+na*na+nb*nb,
...
int main(int argc,char**argv){
    const std::filesystem::path outDir=argc>1?argv[1]:"build/orthogonal-force-compliance";
    ...
    out<<"truth_id,baseline,repair,baseline_holdout_m,repair_holdout_m,baseline_target_m,repair_target_m,label,"
          ...
          "force_progress_z,force_decision,force_num_baseline_m,force_num_repair_m,"
          "raw_point,fisher_point,maxmotion_point,provenance\n";

    const std::array<double,3> truthEs{14125.,16625.,18125.};
    const std::array<double,2> truthNus{.22,.32};
    ...
    for(double truthE:truthEs)for(double truthNu:truthNus){
        ...
        auto truthForce=simulateForceBundle(truthE,truthNu,MpmTransferScheme::APIC,kTruthDt);
        for(std::size_t i=0;i<truthForce.size();++i)
            truthForce[i]=addNoise(std::move(truthForce[i]),truthId*431+static_cast<int>(i));
        ...
        for(const auto&v:variants){
            ...
            c.forceResponses=simulateForceBundle(c.fit.E,c.fit.nu,v.scheme,v.dt);
            c.refinedForceResponses=simulateForceBundle(c.fit.E,c.fit.nu,v.scheme,v.dt*.5);
        ...
        for(std::size_t i=0;i<candidates.size();++i){
            for(std::size_t j=i+1;j<candidates.size();++j){
                ...
                const double zForce=forceProgress(*base,*repair,truthForce);
                const double nForceBase=datasetRms(base->forceResponses,base->refinedForceResponses);
                const double nForceRepair=datasetRms(repair->forceResponses,repair->refinedForceResponses);
                ...
                out<<truthId<<','<<base->variant.name<<','<<repair->variant.name<<','
                   ...
                   <<zMax<<','<<decision(zMax)<<','
                   <<zForce<<','<<decision(zForce)<<','<<nForceBase<<','<<nForceRepair<<','
                   <<rawPoint<<','<<fisherPoint<<','<<maxPoint
                   <<",synthetic-orthogonal-force-compliance\n";
                ++proposalCount;
            }
        }
    }
    ...
    meta<<"{\n"
        <<"  \"schema\": \"vulkax.orthogonal_force_compliance\",\n"
        <<"  \"version\": 1,\n"
        <<"  \"provenance\": \"synthetic-orthogonal-force-compliance\",\n"
        <<"  \"truth_count\": "<<truthId<<",\n"
        <<"  \"proposal_count\": "<<proposalCount<<",\n"
        <<"  \"selection_labels_used\": false,\n"
        <<"  \"decision_threshold_abs_z\": "<<kDecisionZ<<",\n"
        <<"  \"protocol\": \"research/benchmarks/ORTHOGONAL_FORCE_COMPLIANCE_PROTOCOL_2026-09-21.md\",\n"
        <<"  \"warning\": \"Fresh orthogonal-information test; protocol frozen before execution. Do not tune this partition.\"\n"
        <<"}\n";
}
