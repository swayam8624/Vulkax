...
constexpr double kTruthDt=1.0e-4,kHorizon=0.008,kNoise=2.2e-5;
const std::array<double,5> kTimes{0.0013,0.0031,0.0047,0.0063,0.0079};
const std::array<std::size_t,2> kMarkers{18U,47U};
...
int main(int argc,char**argv){
    const std::filesystem::path dir=argc>1?argv[1]:"build/refusal-validation3";std::filesystem::create_directories(dir);std::ofstream out(dir/"cases.csv");
    ...
    const Matrix3 train{1.026,.011,.007,.006,.981,.013,.003,.006,.992};
    const std::array<Intv,4> evidence{{{"diag_shear",{1.030,.070,.026,.018,.982,.041,.022,.034,1.008}},
      {"yz_mix",{1.052,.018,.021,.032,.962,.071,.016,.044,1.014}},
      {"biax_skew",{1.067,.039,.057,.021,.947,.031,.046,.019,1.019}},
      {"offaxis3",{1.044,.066,.013,.055,.958,.037,.024,.063,1.011}}}};
    const Matrix3 target{1.094,.086,.052,.037,.927,.083,.049,.061,1.027};
    ...
    for(double E:truthEs)for(double nu:truthNus){++tid;const auto tr=sim(E,nu,train,kTruthDt,MpmTransferScheme::APIC);const auto fo=noisy(obs(tr,0,2),2701+tid),ho=noisy(obs(tr,2,5),2901+tid);
      for(V v:vars){const auto before=search(v,train,fo,ho);const Intv*sel=&evidence[0];Spectrum ss{};double best=-1;
        ...
        const auto evObs=noisy(obs(sim(E,nu,sel->d,kTruthDt,MpmTransferScheme::APIC),0,5),3101+tid*23+static_cast<int>(v));
        ...
        ++cid;out<<cid<<','<<tid<<','<<vname(v)<<','<<sel->name<<','<<std::setprecision(17)<<E<<','<<nu<<','<<before.E<<','<<before.nu<<','<<after.E<<','<<after.nu<<','<<before.fit<<','<<before.held<<','<<after.fit<<','<<after.held<<','<<after.ev<<','<<after.ev/kNoise<<','<<after.held/kNoise<<','<<ss.smin<<','<<ss.cond<<','<<pre<<','<<post<<','<<nc.coarse<<','<<nc.fine<<','<<nc.ratio<<','<<schFrac(after,target)<<','<<(post<=.10?1:0)<<",synthetic-validation3\n";
      }}
    out.close();std::ofstream meta(dir/"summary.json");meta<<"{\"schema\":\"vulkax.refusal_repair_validation3\",\"version\":1,\"provenance\":\"synthetic-validation3\",\"case_count\":"<<cid<<",\"measurement_noise_m\":"<<std::setprecision(17)<<kNoise<<",\"truth_grid\":\"fresh off-grid values\",\"interventions\":\"fresh train/evidence/target domain; no Validation-1/2 labels used\"}\n";std::cout<<"generated_validation3_cases="<<cid<<"\n";
}
