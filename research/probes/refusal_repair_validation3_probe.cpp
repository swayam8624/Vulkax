...
constexpr double kNoise=3.0e-5;
constexpr double kNumericalBudget=0.05;
const std::array<double,5> kTimes{0.0013,0.0027,0.0042,0.0061,0.0079};
const std::array<std::size_t,2> kMarkers{12U,49U};
...
int main(int argc,char**argv){
    const std::filesystem::path dir=argc>1?argv[1]:"build/refusal-validation3";
    ...
    const Matrix3 train{1.017,.021,.006,.006,.982,.013,.008,.004,.991};
    const std::array<Intv,4> evidence{{{"skew_a",{1.028,.071,.029,.034,.971,.019,.021,.044,1.006}},
      {"skew_b",{1.052,.033,.064,.019,.956,.041,.035,.011,1.019}},
      {"triax_b",{1.063,.041,.017,.054,.944,.033,.016,.049,1.022}},
      {"mixed_b",{1.081,.062,.038,.031,.929,.067,.045,.019,1.027}}}};
    const Matrix3 target{1.094,.126,.041,.063,.903,.074,.077,.036,1.041};
    const std::array<double,4> truthEs{12750,14250,15750,17250};
    const std::array<double,4> truthNus{.225,.275,.325,.375};
    ...
    for(double E:truthEs)for(double nu:truthNus){
        ...
        const auto fo=noisy(obs(tr,0,2),3101+tid),ho=noisy(obs(tr,2,5),3301+tid);
        for(V v:vars){
            ...
            const auto evObs=noisy(obs(sim(E,nu,sel->d,kTruthDt,MpmTransferScheme::APIC),0,5),3501+tid*23+static_cast<int>(v));
            ...
            out<<cid<<','<<tid<<','<<vname(v)<<','<<sel->name<<','<<std::setprecision(17)
               ...
               <<(core?1:0)<<','<<(plus?1:0)<<','<<(post<=.10?1:0)<<",synthetic-validation3\n";
        }
    }
    out.close();
    std::ofstream meta(dir/"summary.json");
    meta<<"{\n  \"schema\":\"vulkax.refusal_repair_validation3\",\n  \"version\":1,\n"
