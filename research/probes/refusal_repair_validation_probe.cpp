...
constexpr double kTruthDt=1.0e-4,kHorizon=0.008,kNoise=2.0e-5;
...
int main(int argc,char**argv){
    const std::filesystem::path dir=argc>1?argv[1]:"build/refusal-validation";std::filesystem::create_directories(dir);std::ofstream out(dir/"cases.csv");
    ...
    const Matrix3 train{1.027,.011,.002,.001,.981,.006,.002,.001,.992};
    const std::array<Intv,4> evidence{{{"yz_shear",{1.,0,.01,0,1.,.095,.01,.025,1.}},{"biaxial",{1.05,.01,0,.01,.955,.015,0,.01,1.02}},
      {"off_axis_2",{1.035,.055,.02,.025,.965,.045,.035,.005,1.01}},{"skew_mix",{1.07,.025,.065,.055,.945,.015,.015,.045,1.015}}}};
    const Matrix3 target{1.095,.085,.035,.035,.925,.070,.055,.015,1.025};
    const std::array<double,4> truthEs{12750,14250,15750,17250};const std::array<double,4> truthNus{.225,.275,.325,.375};
    const std::array<V,4> vars{V::Fine,V::Coarse,V::PIC,V::Constrained};int tid=0,cid=0;
    for(double E:truthEs)for(double nu:truthNus){++tid;const auto tr=sim(E,nu,train,kTruthDt,MpmTransferScheme::APIC);const auto fo=noisy(obs(tr,0,2),701+tid),ho=noisy(obs(tr,2,5),901+tid);
      for(V v:vars){const auto before=search(v,train,fo,ho);const Intv*sel=&evidence[0];Spectrum ss{};double best=-1;
        ...
        const auto evObs=noisy(obs(sim(E,nu,sel->d,kTruthDt,MpmTransferScheme::APIC),0,5),1101+tid*13+static_cast<int>(v));
    ...
    out.close();std::ofstream meta(dir/"summary.json");meta<<"{\"schema\":\"vulkax.refusal_repair_validation\",\"version\":1,\"provenance\":\"synthetic-validation\",\"case_count\":"<<cid<<",\"truth_grid\":\"off-grid from fitted parameter grid\",\"interventions\":\"disjoint from calibration probe\"}\n";std::cout<<"generated_validation_cases="<<cid<<"\n";
}
