...
constexpr double kTruthDt=1.0e-4,kHorizon=0.008,kNoise=2.5e-5;
const std::array<double,5> kTimes{0.0014,0.0029,0.0045,0.0062,0.0078};
const std::array<std::size_t,2> kMarkers{5U,58U};
...
std::vector<double> noisy(std::vector<double> y,int salt){for(std::size_t i=0;i<y.size();++i){const double q=.811*(i+1)+1.337*(salt+1);y[i]+=kNoise*(.61*std::sin(q)+.39*std::cos(1.731*q));}return y;}
...
double numFrac(const Fit&f,const Matrix3&t){
    const auto a=obs(sim(f.E,f.nu,t,f.dt,f.scheme),0,5),b=obs(sim(f.E,f.nu,t,.5*f.dt,f.scheme),0,5);
    return rms(a,b)/std::max(rms(a,initialObs(t)),1e-15);
}
...
int main(int argc,char**argv){
    const std::filesystem::path dir=argc>1?argv[1]:"build/refusal-validation2";std::filesystem::create_directories(dir);std::ofstream out(dir/"cases.csv");
    out<<"case_id,truth_id,variant,selected_evidence,truth_E_pa,truth_nu,initial_E_pa,initial_nu,repaired_E_pa,repaired_nu,initial_fit_rms_m,initial_heldout_rms_m,repaired_fit_rms_m,repaired_heldout_rms_m,evidence_rms_m,evidence_noise_ratio,selected_smin,selected_condition,pre_target_relative_error,post_target_relative_error,numerical_fraction,scheme_fraction,post_safe_10pct,provenance\n";
    const Matrix3 train{1.021,.016,.004,.003,.976,.009,.004,.002,.995};
    const std::array<Intv,4> evidence{{{"xy_twist",{1.015,.082,.018,.028,.985,.025,.012,.052,1.004}},
      {"tri_axial",{1.045,.025,.012,.018,.972,.036,.028,.016,1.012}},
      {"skew3",{1.058,.018,.072,.047,.952,.022,.019,.038,1.020}},
      {"mixed2",{1.075,.048,.032,.061,.938,.028,.011,.057,1.018}}}};
    const Matrix3 target{1.082,.112,.026,.052,.918,.061,.068,.028,1.032};
    const std::array<double,4> truthEs{13125,14625,16125,17625};const std::array<double,4> truthNus{.2125,.2625,.3375,.3875};
    const std::array<V,4> vars{V::Fine,V::Coarse,V::PIC,V::Constrained};int tid=0,cid=0;
    for(double E:truthEs)for(double nu:truthNus){++tid;const auto tr=sim(E,nu,train,kTruthDt,MpmTransferScheme::APIC);const auto fo=noisy(obs(tr,0,2),1701+tid),ho=noisy(obs(tr,2,5),1901+tid);
      for(V v:vars){const auto before=search(v,train,fo,ho);const Intv*sel=&evidence[0];Spectrum ss{};double best=-1;
        ...
        const auto evObs=noisy(obs(sim(E,nu,sel->d,kTruthDt,MpmTransferScheme::APIC),0,5),2101+tid*19+static_cast<int>(v));
        ...
        ++cid;out<<cid<<','<<tid<<','<<vname(v)<<','<<sel->name<<','<<std::setprecision(17)<<E<<','<<nu<<','<<before.E<<','<<before.nu<<','<<after.E<<','<<after.nu<<','<<before.fit<<','<<before.held<<','<<after.fit<<','<<after.held<<','<<after.ev<<','<<after.ev/kNoise<<','<<ss.smin<<','<<ss.cond<<','<<pre<<','<<post<<','<<numFrac(after,target)<<','<<schFrac(after,target)<<','<<(post<=.10?1:0)<<",synthetic-validation\n";
      }}
    out.close();std::ofstream meta(dir/"summary.json");meta<<"{\"schema\":\"vulkax.refusal_repair_validation2\",\"version\":1,\"provenance\":\"synthetic-validation\",\"case_count\":"<<cid<<",\"truth_grid\":\"off-grid from fitted parameter grid\",\"interventions\":\"disjoint from calibration probe\"}\n";std::cout<<"generated_validation2_cases="<<cid<<"\n";
}
