...
class GeoBeaconToolsTests(unittest.TestCase):
    ...
    def test_midtown_manhattan_dataset_glbs_reload(self):
        root = ROOT / "data/midtown_manhattan"
        manifest_path = root / "generated/geobeacon.json"
        manifest = json.loads(manifest_path.read_text())
        report = json.loads((root / "generated/tile_report.json").read_text())
        self.assertEqual(manifest["datasetId"], "midtown-manhattan")
        self.assertEqual(manifest["displayName"], "Midtown Manhattan")
        self.assertGreater(len(manifest["tiles"]), 200)
        self.assertGreater(report["featureCount"], 6000)
        self.assertEqual(report["invalidFeatures"], {})
        self.assertEqual(
            hashlib.sha256((root / "source.osm").read_bytes()).hexdigest(),
            manifest["sourceChecksum"],
    ...
    def test_checked_city_registry(self):
        ...
        for city in registry["cities"]:
            ...
            self.assertEqual(len(city["camera"]["position"]), 3)
            self.assertEqual(len(city["camera"]["rotation"]), 3)
            self.assertLess(city["camera"]["position"][1], 0.0)
            manifest = json.loads(
