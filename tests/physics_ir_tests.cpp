...
int main() {
  ...
  model.fields.push_back(
      {"velocity", ValueType::Scalar, Dimension::dimensionless(), FieldPlacement::CellCenter});
  ...
  const auto reflected = reflectResourceLayout(*parsed.model, *graph);
  assert(reflected.resources.size() >= parsed.model->fields.size());
  assert(reflected.passes.size() == graph->passes.size());
  assert(reflected.estimatedBytes > 0);
  assert(reflected.canonicalHash != 0);
  for (size_t binding = 0; binding < reflected.resources.size(); ++binding) {
    assert(reflected.resources[binding].binding == binding);
  }
  const auto vulkanPlan = makeVulkanResourcePlan(reflected);
  assert(vulkanPlan.bindings.size() == reflected.resources.size());
  assert(vulkanPlan.canonicalHash == reflected.canonicalHash);
  assert(!vulkanPlan.poolSizes.empty());
  for (size_t binding = 0; binding < vulkanPlan.bindings.size(); ++binding) {
    assert(vulkanPlan.bindings[binding].binding == binding);
    assert(vulkanPlan.bindings[binding].descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_IMAGE);
    assert(vulkanPlan.bindings[binding].imageFormat != VK_FORMAT_UNDEFINED);
  }

  const auto cacheDirectory = std::filesystem::temp_directory_path() / "vulkax-pipeline-cache-test";
  std::filesystem::remove_all(cacheDirectory);
  PipelineArtifactCache cache(cacheDirectory);
  PipelineCacheKey cacheKey{
      graph->canonicalHash,
      reflected.canonicalHash,
      "vulkan",
      "test-device",
      "test-compiler"};
  PipelineArtifact artifact{cacheKey, {std::byte{1}, std::byte{2}, std::byte{3}}};
  cache.store(artifact);
  const auto loadedArtifact = cache.load(cacheKey);
  assert(loadedArtifact.has_value());
  assert(loadedArtifact->bytes == artifact.bytes);
  PipelineCacheKey changedKey = cacheKey;
  changedKey.deviceIdentity = "different-device";
  assert(!cache.load(changedKey).has_value());
  cache.erase(cacheKey);
  assert(!cache.load(cacheKey).has_value());
  std::filesystem::remove_all(cacheDirectory);
  const auto repeatedGraph = lowerToSolverGraph(*parsed.model);
  assert(repeatedGraph->canonicalHash == graph->canonicalHash);

  const auto explicitGraphModel = parsePhysicsDsl(R"(
    model equation-defined-flow
    field velocity: vector3 [m/s] face_x
    field density: scalar [1] cell
    domain -1 -1 -1 1 1 1
    resolution 16 16 16
    initial velocity = 0
    initial density = 0
    pass move_density advect_scalar reads velocity,density writes density_next
    pass curl_velocity compute_curl reads velocity writes curl
    solver advection maccormack pressure multigrid timestep 0.01
  )");
  assert(explicitGraphModel.valid());
  const auto explicitGraph = lowerToSolverGraph(*explicitGraphModel.model);
  assert(explicitGraph.has_value());
  assert(explicitGraph->passes.size() == 2);
  assert(explicitGraph->passes[0].name == "move_density");
  assert(explicitGraph->passes[1].writes == std::vector<std::string>{"curl"});

  auto invalidExplicit = *explicitGraphModel.model;
  invalidExplicit.passes.front().reads.push_back("missing_resource");
  std::vector<ValidationIssue> explicitIssues;
  assert(!lowerToSolverGraph(invalidExplicit, &explicitIssues).has_value());
  assert(!explicitIssues.empty());

  const auto coupledModel = makeFluidRigidInteractionModel();
  const auto coupledGraph = lowerToSolverGraph(coupledModel);
  assert(coupledGraph.has_value());
  assert(
      std::any_of(
          coupledGraph->passes.begin(),
          coupledGraph->passes.end(),
          [](const SolverPass& pass) { return pass.kind == SolverPassKind::VoxelizeObstacles; }));
  assert(
      std::any_of(
          coupledGraph->passes.begin(),
          coupledGraph->passes.end(),
          [](const SolverPass& pass) {
            return pass.kind == SolverPassKind::IntegrateFluidForces;
          }));
  const auto coupledLayout = reflectResourceLayout(coupledModel, *coupledGraph);
  const auto coupledVulkanPlan = makeVulkanResourcePlan(coupledLayout);
  assert(
      std::any_of(
          coupledVulkanPlan.bindings.begin(),
          coupledVulkanPlan.bindings.end(),
          [](const VulkanResourceBinding& binding) {
            return binding.descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
          }));
  assert(
      std::any_of(
          coupledLayout.resources.begin(),
          coupledLayout.resources.end(),
          [](const ReflectedResource& resource) {
            return resource.name == "solid_mask" && resource.format == ResourceFormat::R32Uint;
          }));
  ...
  const auto invalidInitial =
      parsePhysicsDsl("model invalid\nfield density: scalar [1] cell\ninitial missing = 0\n");
  ...
  fieldModel.fields = {
      {"height", ValueType::Scalar, Dimension::length(), FieldPlacement::CellCenter}};
  ...
  const double sample =
      executeScalarProgram(*compute.program, {0.75, 0.0, 0.0}, 0.5, waveParameters);
  ...
  const auto optimized = lowerScalarFieldProgram(
      fieldModel,
      "height",
      vulkax::equation::parseScalarExpression("x * x + x * x + (2 + 3)"));
  assert(optimized.valid());
  const auto multiplyCount = std::count_if(
      optimized.program->instructions.begin(),
      optimized.program->instructions.end(),
      [](const ScalarInstruction& instruction) {
        return instruction.opcode == ScalarOpcode::Multiply;
      });
  ...
  assert(
      std::any_of(
          ...
          optimized.program->instructions.end(),
          [](const ScalarInstruction& instruction) {
            return instruction.opcode == ScalarOpcode::Constant &&
                   std::abs(instruction.immediate - 5.0) < 1e-12;
          }));

  const auto unbound = lowerScalarFieldProgram(
      fieldModel,
      "height",
      vulkax::equation::parseScalarExpression("missing * x"));
  assert(!unbound.valid());
  fieldModel.fields.front().valueType = ValueType::Vector3;
  assert(!lowerScalarFieldProgram(
              fieldModel,
              "height",
              waveExpression,
              {"amplitude", "wavenumber", "angularFrequency"})
  ...
  diffusionModel.fields = {
      {"u", ValueType::Scalar, Dimension::dimensionless(), FieldPlacement::CellCenter}};
  ...
  const auto diffusion = lowerScalarEvolutionProgram(
      diffusionModel,
      "u",
      vulkax::equation::parseScalarExpression("diffusivity * laplacian(u) - decay * u"),
      {"diffusivity", "decay"});
  ...
  const double dx =
      (diffusionModel.domain.maximum[0] - diffusionModel.domain.minimum[0]) / extent[0];
  ...
  assert(
      std::abs(
          std::accumulate(evolved.begin(), evolved.end(), 0.0) -
  ...
  const auto fixed = lowerScalarEvolutionProgram(
      fixedModel,
      "u",
      vulkax::equation::parseScalarExpression("laplacian(u)"));
  assert(fixed.valid());
  ...
  assert(!lowerScalarEvolutionProgram(
              diffusionModel,
              "u",
              vulkax::equation::parseScalarExpression("laplacian(missing)"))
              .valid());
  ...
  const auto reaction = lowerCoupledScalarEvolutionProgram(
      ...
      {
          {"a",
           vulkax::equation::parseScalarExpression(
  ...
  assert(
      std::all_of(reactionOutput.at("a").begin(), reactionOutput.at("a").end(), [&](float value) {
        return std::abs(value - expectedA) < 1e-6;
      }));
  assert(
      std::all_of(reactionOutput.at("b").begin(), reactionOutput.at("b").end(), [&](float value) {
        return std::abs(value - expectedB) < 1e-6;
      }));
  ...
  assert(!lowerCoupledScalarEvolutionProgram(
              ...
              {{"a", vulkax::equation::parseScalarExpression("laplacian(missing)")},
               {"b", vulkax::equation::parseScalarExpression("a")}})
              .valid());
