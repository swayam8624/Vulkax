...
The locked decision-level result also reproduces on an independent Ubuntu/current-dependency replay. The replay completes development, validation, and the reserved 10-video final campaign with the same discrete final outcomes: 90/110 primary versus 40/110 baseline correct decisions, 40/50 SUPPORT, 40/50 VETO, 10/10 placebo UNRESOLVED, and paired counts 50/0/60. The continuous median period-inferred length relative error shifts slightly from 0.1681086451 in the locked Mac/Metal ledger to 0.1679383811 on Ubuntu (about 0.10% relative), so exact cross-platform floating/numeric identity is not claimed.
...
| Independent Ubuntu pendulum replay | [IRIS_PENDULUM_LINUX_REPLAY_RESULT_2026-09-24.md](research/results/IRIS_PENDULUM_LINUX_REPLAY_RESULT_2026-09-24.md) |
