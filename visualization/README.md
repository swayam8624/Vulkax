...
## Post-final reviewer-hardening visuals

After the locked IRIS final experiment, additional deterministic evidence figures
are generated by:
...
Outputs under `build/visualization/reviewer-hardening/` include:
- per-video clustered accuracy;
- locked result vs stronger post-final baselines;
- corruption robustness envelope;
- GT-hidden proposal-gate summary;
- a measured-video rewrite storyboard;
- an optional measured-video MP4.

These assets are explicitly `post_final_evidence`. They may strengthen or narrow
claims but may not rewrite the frozen confirmatory result.
