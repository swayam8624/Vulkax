...
def main():
 ...
 if not cand:raise SystemExit("no ordinary-improving blind proposal available for demo")
 ...
 panels=[]
 stages=[(.20,"01 OBSERVE","baseline from first 20%"),(.45,"02 REPAIR","candidate from first 40%; ordinary holdout checks improvement"),(.80,"03 VERIFY","disjoint later evidence decides SUPPORT / VETO / UNRESOLVED")]
 ...
 if not a.no_video:
  ...
  while True:
   ...
   stage="OBSERVE" if u<.20 else ("PROPOSE REPAIR" if u<.50 else "VERIFY ON DISJOINT LATE EVIDENCE")
   cv2.rectangle(fr,(0,0),(640,62),(10,15,25),-1)
   cv2.putText(fr,stage,(16,27),cv2.FONT_HERSHEY_SIMPLEX,.65,(255,255,255),2,cv2.LINE_AA)
   if u>=.50:cv2.putText(fr,f"score {float(r['standardized_evidence_score']):.2f} -> {r['decision'].upper()}",(16,52),cv2.FONT_HERSHEY_SIMPLEX,.48,(0,230,255),1,cv2.LINE_AA)
   writer.write(fr);i+=1
